...
def solve_nqueens_seq(board, col, solutions, moves):
    ...
    for i in range(len(board)):
        if is_safe(board, i, col):
            board[i][col] = 1
            moves.append((i, col, True))  # Add move to list for animation
            solve_nqueens_seq(board, col + 1, solutions, moves)
            board[i][col] = 0
            moves.append((i, col, False))  # Remove move for backtracking in animation

def n_queens_sequential(n):
    board = [[0] * n for _ in range(n)]
    solutions = []
    moves = []  # Track moves for animation
...
def thread_worker(n, col, board, solutions, lock, moves):
    ...
    for i in range(n):
        if is_safe(board, i, col):
            ...
            board_copy[i][col] = 1
            with lock:
                moves.append((i, col, True))  # Add move for animation
            thread_worker(n, col + 1, board_copy, solutions, lock, moves)
            with lock:
                moves.append((i, col, False))  # Remove move for backtracking
...
n = 10  # Change N value to test other sizes
