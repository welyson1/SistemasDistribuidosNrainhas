...
def main():
    ...
    for n in n_values:
        ...
        # Calcula a escalabilidade
        scalability = speedup * par_solver.num_processes
        scalabilities.append(scalability)
        print(f"Escalabilidade: {scalability:.2f}")
    ...
    # Gera o terceiro gráfico (Escalabilidade)
    plt.figure(figsize=(12, 6))

    # Gráfico de Escalabilidade
    plt.plot(n_values, scalabilities, marker='o', label='Escalabilidade', color='purple')
    plt.title('Escalabilidade da Solução Paralela')
    plt.xlabel('Tamanho do Tabuleiro (N)')
    plt.ylabel('Escalabilidade')
    plt.xticks(n_values)
    plt.grid(True)
    plt.legend()
